...
module("luci.controller.easserver2", package.seeall)
...
function index()
	if not nixio.fs.access("/etc/config/easserver2") then
		if nixio.fs.access("/usr/share/easserver2/0_default_config") then
			luci.sys.call('cp -f /usr/share/easserver2/0_default_config /etc/config/easserver2')
		else return end
	end
	local appname = "easserver2"			-- global definitions not available
...
--	entry({"admin", "services", appname, "hide"}, call("hide_menu")).leaf = true
	...
	if uci:get(appname, "@global[0]", "hide_from_luci") ~= "1" then
		e = entry({"admin", "services", appname}, alias("admin", "services", appname, "settings"), _("EAS Server路由系统配置"), 0)
	else
		e = entry({"admin", "services", appname}, alias("admin", "services", appname, "settings"), nil, 0)
	end
	e.dependent = true
	e.acl_depends = { "luci-app-easserver2" }
	--[[ Client ]]
	entry({"admin", "services", appname, "settings"}, cbi(appname .. "/client/global"), _("EAS Server系统状态"), 1).dependent = true
	entry({"admin", "services", appname, "node_list"}, cbi(appname .. "/client/node_list"), _("网络列表"), 2).dependent = true
...
--	entry({"admin", "services", appname, "node_subscribe_config"}, cbi(appname .. "/client/node_subscribe_config")).leaf = true
	...
	entry({"admin", "services", appname, "acl"}, cbi(appname .. "/client/acl"), _("WIFI配置列表"), 98).leaf = true
	entry({"admin", "services", appname, "acl_config"}, cbi(appname .. "/client/acl_config")).leaf = true
--	entry({"admin", "services", appname, "log"}, form(appname .. "/client/log"), _("日志"), 999).leaf = true
...
--	entry({"admin", "services", appname, "server_user"}, cbi(appname .. "/server/user")).leaf = true
	...
	--[[Components update]]
	entry({"admin", "services", appname, "check_easserver2"}, call("app_check")).leaf = true
	local coms = require "luci.easserver2.com"
	local com
...
function reset_config()
	luci.sys.call('/etc/init.d/easserver2 stop')
	luci.sys.call('[ -f "/usr/share/easserver2/0_default_config" ] && cp -f /usr/share/easserver2/0_default_config /etc/config/easserver2')
...
function link_add_node()
	...
	luci.sys.call("lua /usr/share/easserver2/subscribe.lua add log")
end
...
function get_now_use_node()
	local e = {}
	local data, code, msg = nixio.fs.readfile("/tmp/etc/easserver2/acl/default/global.id")
	if data then
...
function get_redir_log()
	...
	local file_path = "/tmp/etc/easserver2/acl/" .. id .. "/" .. name .. ".log"
...
function get_socks_log()
	local name = luci.http.formvalue("name")
	local path = "/tmp/etc/easserver2/SOCKS_" .. name .. ".log"
...
function get_log()
	-- luci.sys.exec("[ -f /tmp/log/easserver2.log ] && sed '1!G;h;$!d' /tmp/log/easserver2.log > /tmp/log/easserver2_show.log")
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/easserver2.log' ] && cat /tmp/log/easserver2.log"))
end

function clear_log()
	luci.sys.call("echo '' > /tmp/log/easserver2.log")
end

function index_status()
	local e = {}
	e["global_status"] = luci.sys.call("/bin/busybox top -bn1 | grep -v 'grep' | grep '/tmp/etc/easserver2/bin/' | grep 'default' | grep 'global' >/dev/null") == 0
	luci.http.prepare_content("application/json")
	luci.http.write_json(e)
end
...
function urltest_node()
	...
	local e = {}
	e.index = index
	local result = luci.sys.exec(string.format("/usr/share/easserver2/test.sh url_test_node %s %s", id, "urltest_node"))
	local code = tonumber(luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $1}'") or "0")
	if code ~= 0 then
		local use_time = luci.sys.exec("echo -n '" .. result .. "' | awk -F ':' '{print $2}'")
		if use_time:find("%.") then
...
function set_node()
	...
	uci:commit(appname)
	luci.sys.call("/etc/init.d/easserver2 restart > /dev/null 2>&1 &")
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/easserver2/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
	http_write_json()
end
...
function server_user_log()
	local id = luci.http.formvalue("id")
	if nixio.fs.access("/tmp/etc/easserver2_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /tmp/etc/easserver2_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		luci.http.write(content)
	else
		luci.http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	luci.http.write(luci.sys.exec("[ -f '/tmp/log/easserver2_server.log' ] && cat /tmp/log/easserver2_server.log"))
end

function server_clear_log()
	luci.sys.call("echo '' > /tmp/log/easserver2_server.log")
end
