module("luci.easserver2.api", package.seeall)
local com = require "luci.easserver2.com"
...
LOG_FILE = "/tmp/log/easserver2.log"
CACHE_PATH = "/tmp/etc/easserver2_tmp"
...
function curl_proxy(url, file, args)
	--使用代理
	local socks_server = luci.sys.exec("[ -f /tmp/etc/easserver2/acl/default/SOCKS_server ] && echo -n $(cat /tmp/etc/easserver2/acl/default/SOCKS_server) || echo -n ''")
...
local function get_bin_version_cache(file, cmd)
	sys.call("mkdir -p /tmp/etc/easserver2_tmp")
	if fs.access(file) then
		chmod_755(file)
		local md5 = sys.exec("echo -n $(md5sum " .. file .. " | awk '{print $1}')")
		if fs.access("/tmp/etc/easserver2_tmp/" .. md5) then
			return sys.exec("echo -n $(cat /tmp/etc/easserver2_tmp/%s)" % md5)
		else
			local version = sys.exec(string.format("echo -n $(%s %s)", file, cmd))
			if version and version ~= "" then
				sys.call("echo '" .. version .. "' > " .. "/tmp/etc/easserver2_tmp/" .. md5)
				return version
			end
		end
	end
...
function to_move(app_name,file)
	...
	local flag = sys.call('pgrep -af "easserver2/.*'.. app_name ..'" >/dev/null')
	if flag == 0 then
		sys.call("/etc/init.d/easserver2 stop")
	end
	...
	sys.call(cmd_rm_tmp)
	if flag == 0 then
		sys.call("/etc/init.d/easserver2 restart >/dev/null 2>&1 &")
	end
...
function get_version()
	local version = sys.exec("opkg list-installed luci-app-easserver2 2>/dev/null | awk '{print $3}'")
	if not version or #version == 0 then
		version = sys.exec("apk info luci-app-easserver2 2>/dev/null | awk 'NR == 1 {print $1}' | cut -d'-' -f4-")
	end
	return version or ""
end

function to_check_self()
	local url = "https://raw.githubusercontent.com/yuyu913gzs/openwrt-passwall2/main/luci-app-passwall2/Makefile"
	local tmp_file = "/tmp/easserver2_makefile"
	...
	local remote_version = sys.exec("echo -n $(grep 'PKG_VERSION' /tmp/easserver2_makefile|awk -F '=' '{print $2}')")
				.. "-" ..  sys.exec("echo -n $(grep 'PKG_RELEASE' /tmp/easserver2_makefile|awk -F '=' '{print $2}')")
