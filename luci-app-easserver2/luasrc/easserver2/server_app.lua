...
local CONFIG = "easserver2_server"
...
local require_dir = "luci.easserver2."

local ipt_bin = sys.exec("echo -n $(/usr/share/easserver2/iptables.sh get_ipt_bin)")
local ip6t_bin = sys.exec("echo -n $(/usr/share/easserver2/iptables.sh get_ip6t_bin)")
...
local function gen_include()
	...
	local function extract_rules(n, a)
		...
		local result = "*" .. a
		result = result .. "\n" .. sys.exec(_ipt .. '-save -t ' .. a .. ' | grep "EAS2-SERVER" | sed -e "s/^-A \\(INPUT\\)/-I \\1 1/"')
	...
	if f and err == nil then
		if nft_flag == "0" then
			f:write(ipt_bin .. '-save -c | grep -v "EAS2-SERVER" | ' .. ipt_bin .. '-restore -c' .. "\n")
			f:write(ipt_bin .. '-restore -n <<-EOT' .. "\n")
			f:write(extract_rules("4", "filter") .. "\n")
			f:write("EOT" .. "\n")
			f:write(ip6t_bin .. '-save -c | grep -v "EAS2-SERVER" | ' .. ip6t_bin .. '-restore -c' .. "\n")
...
local function start()
	...
	if nft_flag == "0" then
		ipt("-N EAS2-SERVER")
		ipt("-I INPUT -j EAS2-SERVER")
		ip6t("-N EAS2-SERVER")
		ip6t("-I INPUT -j EAS2-SERVER")
	else
		nft_file, err = io.open(NFT_INCLUDE_FILE, "w")
		nft_file:write('#!/usr/sbin/nft -f\n')
		nft_file:write('add chain inet fw4 EAS2-SERVER\n')
		nft_file:write('flush chain inet fw4 EAS2-SERVER\n')
		nft_file:write('insert rule inet fw4 input position 0 jump EAS2-SERVER comment "EAS2-SERVER"\n')
	end
	uci:foreach(CONFIG, "user", function(user)
		...
		if enable and tonumber(enable) == 1 then
			...
			if bind_local and tonumber(bind_local) ~= 1 then
				if nft_flag == "0" then
					ipt(string.format('-A EAS2-SERVER -p tcp --dport %s -m comment --comment "%s" -j ACCEPT', port, remarks))
					ip6t(string.format('-A EAS2-SERVER -p tcp --dport %s -m comment --comment "%s" -j ACCEPT', port, remarks))
					if udp_forward == 1 then
						ipt(string.format('-A EAS2-SERVER -p udp --dport %s -m comment --comment "%s" -j ACCEPT', port, remarks))
						ip6t(string.format('-A EAS2-SERVER -p udp --dport %s -m comment --comment "%s" -j ACCEPT', port, remarks))
					end
				else
					nft_file:write(string.format('add rule inet fw4 EAS2-SERVER meta l4proto tcp tcp dport {%s} counter accept comment "%s"\n', port, remarks))
					if udp_forward == 1 then
						nft_file:write(string.format('add rule inet fw4 EAS2-SERVER meta l4proto udp udp dport {%s} counter accept comment "%s"\n', port, remarks))
					end
				end
			end
		end
	...
	if nft_flag == "1" then
		nft_file:write("add rule inet fw4 EAS2-SERVER return\n")
		nft_file:close()
...
local function stop()
	...
	if nft_flag == "0" then
		ipt("-D INPUT -j EAS2-SERVER 2>/dev/null")
		ipt("-F EAS2-SERVER 2>/dev/null")
		ipt("-X EAS2-SERVER 2>/dev/null")
		ip6t("-D INPUT -j EAS2-SERVER 2>/dev/null")
		ip6t("-F EAS2-SERVER 2>/dev/null")
		ip6t("-X EAS2-SERVER 2>/dev/null")
	else
		local nft_cmd = "handles=$(nft -a list chain inet fw4 input | grep -E \"EAS2-SERVER\" | awk -F '# handle ' '{print$2}')\n for handle in $handles; do\n nft delete rule inet fw4 input handle ${handle} 2>/dev/null\n done"
		cmd(nft_cmd)
		cmd("nft flush chain inet fw4 EAS2-SERVER 2>/dev/null")
		cmd("nft delete chain inet fw4 EAS2-SERVER 2>/dev/null")
	end
