module("luci.easserver2.util_sing-box", package.seeall)
...
function gen_config_server(node)
	...
	if node.outbound_node and node.outbound_node ~= "nil" then
		...
		if node.outbound_node == "_iface" and node.outbound_node_iface then
			...
			sys.call("mkdir -p /tmp/etc/easserver2/iface && touch /tmp/etc/easserver2/iface/" .. node.outbound_node_iface)
		else
			local outbound_node_t = uci:get_all("easserver2", node.outbound_node)
			...
			outbound = require("luci.easserver2.util_sing-box").gen_outbound(nil, outbound_node_t, "outbound")
		end
		if outbound then
	...
	if node then
				...
				elseif _node_id ~= "nil" then
					...
					elseif _node.protocol == "_iface" then
						if _node.iface then
							...
							table.insert(outbounds, _outbound)
							rule_outboundTag = _outbound.tag
							sys.call("touch /tmp/etc/easserver2/iface/" .. _node.iface)
						end
					end
				end
		...
		elseif node.protocol == "_iface" then
			if node.iface then
				...
				sys.call("touch /tmp/etc/easserver2/iface/" .. node.iface)
			end
		else
	...
	if dns_listen_port then
		...
		if remote_dns_fake then
			...
			experimental.cache_file = {
				enabled = true,
				store_fakeip = true,
				path = "/tmp/singbox_easserver2_" .. flag .. ".db"
			}
		end
	
		if direct_dns_udp_server then
			local domain = {}
			local nodes_domain_text = sys.exec('uci show easserver2 | grep ".address=" | cut -d "\'" -f 2 | grep "[a-zA-Z]$" | sort -u')
