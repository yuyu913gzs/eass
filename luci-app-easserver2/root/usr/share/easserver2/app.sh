...
PROXY_IPV6=0
PROXY_IPV6_UDP=0
LUA_UTIL_PATH=/usr/lib/lua/luci/easserver2
...
lua_api() {
	...
	echo $(lua -e "local api = require 'luci.easserver2.api' print(api.${func})")
}
...
run_xray() {
	...
	[ -n "$dns_listen_port" ] && {
		...
		[ "${write_ipset_direct}" = "1" ] && {
			...
			if [ "${nftflag}" = "1" ]; then
				local direct_nftset="4#inet#easserver2#easserver2_${set_flag}_whitelist,6#inet#easserver2#easserver2_${set_flag}_whitelist6"
			else
				local direct_ipset="easserver2_${set_flag}_whitelist,easserver2_${set_flag}_whitelist6"
			fi
...
start() {
	pgrep -f /tmp/etc/easserver2/bin > /dev/null 2>&1 && {
...
stop() {
	clean_log
	[ -n "$($(source $APP_PATH/iptables.sh get_ipt_bin) -t mangle -t nat -L -nv 2>/dev/null | grep "EAS2")" ] && source $APP_PATH/iptables.sh stop
