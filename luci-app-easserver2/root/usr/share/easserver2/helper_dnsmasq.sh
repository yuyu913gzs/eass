...
add() {
	...
	[ "${NFTFLAG}" = "1" ] && {
		set_type="nftset"
		local setflag_4="4#inet#easserver2#"
		local setflag_6="6#inet#easserver2#"
	}
	
	#始终用国内DNS解析节点域名
	servers=$(uci show "${CONFIG}" | grep ".address=" | cut -d "'" -f 2)
	hosts_foreach "servers" host_from_url | grep '[a-zA-Z]$' | sort -u | grep -v "engage.cloudflareclient.com" | gen_items settype="${set_type}" setnames="${setflag_4}easserver2_vpslist,${setflag_6}easserver2_vpslist6" dnss="${LOCAL_DNS:-${DEFAULT_DNS}}" outf="${TMP_DNSMASQ_PATH}/10-vpslist_host.conf" ipsetoutf="${TMP_DNSMASQ_PATH}/ipset.conf"
