...
listen %s
	...
	external-check command "/usr/share/easserver2/haproxy_check.sh"
]], port, port))
	end
	...
	for i, o in ipairs(listens[port]) do
		...
		if o.export ~= "0" then
			sys.call(string.format("/usr/share/easserver2/app.sh add_ip2route %s %s", o.origin_address, o.export))
		end
